...
def create_user(display_name: str, handle: str | None = None, tenant_id: int | None = None, role: str = "member", meta_json: Any = None, is_global: bool = False, is_global_admin: bool = False, slug: str | None = None) -> dict[str, Any]:
    ...
    with db_session() as conn:
        cur = conn.execute(
            """
            INSERT INTO users(uuid,display_name,handle,slug,tenant_id,is_global,is_global_admin,role,created_at,updated_at,meta_json)
            VALUES(?,?,?,?,?,?,?,?,?,?,?)
            """,
            (new_uuid(), display_name, user_slug, user_slug, tenant_value, global_flag, admin_flag, role_value, now, now, _json(meta_json)),
        )
...
def update_user(user_id: int, patch: dict[str, Any]) -> dict[str, Any]:
    ensure_identity_schema(); allowed={"display_name","handle","slug","tenant_id","is_global","is_global_admin","role","is_enabled","meta_json"}; sets=[]; vals=[]
    patch = dict(patch or {})
