...
* THIS IS DONE 3/10: Start treating memory text the same way I am treating files and chat transcripts, by stuffing it into the artifactor and chunking them if they're really big. That'd mean that a memory could be potentially something as big as let's say one of Alara's 20,000 word short stories and it could still be accessible to the bot as if it remembered reading the whole thing – effectively the same as files are now (though really big PDF do get truncated for sanity).
